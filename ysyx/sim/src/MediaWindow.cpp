...
MediaWindow::MediaWindow() {
  if (SDL_Init(SDL_INIT_VIDEO) < 0) {
    std::cout << rang::fg::red
              << "SDL could not be initialized: " << SDL_GetError()
              << rang::fg::reset << std::endl;
  } else {
    std::cout << rang::fg::yellow << "SDL video system is ready to go"
              << rang::fg::reset << std::endl;
  }

  win = SDL_CreateWindow("PS/2 & VGA Test", SDL_WINDOWPOS_CENTERED,
                         SDL_WINDOWPOS_CENTERED, VGA_H_RES, VGA_V_RES,
                         SDL_WINDOW_SHOWN);

  // NOTE:  need to handle excpt cond
  if (!win) {
    std::cout << rang::fg::red << "Window creation failed: " << SDL_GetError()
              << rang::fg::reset << std::endl;
  }
  rdr = SDL_CreateRenderer(win, -1, SDL_RENDERER_ACCELERATED);
  if (!rdr) {
    std::cout << rang::fg::red << "Renderer creation failed: " << SDL_GetError()
              << rang::fg::reset << std::endl;
  }
}
...
void MediaWindow::calcFPS() {
  ...
  double duration =
      ((double)(endTick - startTick)) / SDL_GetPerformanceFrequency();
...
void MediaWindow::initImage() {
  ...
  if ((initFlags & flags) != flags) {
    std::cout << rang::fg::red << "SDL2 format not available" << rang::fg::reset
              << std::endl;
    return;
  }
  ...
  if (!srf) {
    std::cout << rang::fg::red << "Image not loaded..." << rang::fg::reset
              << std::endl;
    return;
  }
...
void MediaWindow::initVideo() {
  ...
  if (!txr) {
    std::cout << rang::fg::red << "Texture creation failed: " << SDL_GetError()
              << rang::fg::reset << std::endl;
  }
  appInit = true;
}
